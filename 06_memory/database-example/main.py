...
@app.post("/chat", response_model=ChatResponse) # 요청 받을 때의 형태(response)
async def chat(request: ChatReuqest): # 요청 보낼 때의 형태(request)
    try:
        config = {
                "configurable": {
...
async def get_histroy(user_id: str, conversation_id:str):
    try:
